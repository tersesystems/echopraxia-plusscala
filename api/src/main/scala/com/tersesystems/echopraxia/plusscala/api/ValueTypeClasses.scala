...
trait ValueTypeClasses {
  ...
  trait LowPriorityToValueAttributesImplicits {
    ...
    implicit def iterableValueFormat[TV: ValueModificationToValueAttributes]: ToValueAttributes[Iterable[TV]] = new ValueModificationToValueAttributes[Iterable[TV]]() {
   ...
   *   the type param
   */
  final class AsCardinal[-T] extends ValueModificationToValueAttributes[T] {
    override def toValue(v: T): Value[_]                   = Value.nullValue()
