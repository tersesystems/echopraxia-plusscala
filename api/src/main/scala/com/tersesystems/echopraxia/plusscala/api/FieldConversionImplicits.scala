...
trait FieldConversionImplicits { self: ValueTypeClasses =>
  ...
  implicit def tupleToField[TV: ToValue](tuple: (String, TV)): Field = newField(tuple._1, tuple._2)
  ...
  implicit def nameAndValueToField[TV: ToValue: ToName](value: TV): Field =
  ...
  @inline
  private def newField[TV: ToValue](name: String, tv: TV): Field = {
    Utils.newField(name, ToValue(tv), Attributes.empty)
  }
}
